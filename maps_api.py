...
#name = quote('범어사 미륵전-불상이야기')   
#API_KEY='AIzaSyCO8Qwx7jM2MDSgujwE-l9lYbwgTvgCUc0'

def build_addr(name,API_KEY):
...
def get_rating(place_id,API_KEY):
    rating_url = 'https://maps.googleapis.com/maps/api/place/details/json?placeid='+place_id+'&fields=name,rating,formatted_phone_number&key='+API_KEY
    req = Request(rating_url, headers={ 'X-Mashape-Key': API_KEY })
    ssltext = ssl.SSLContext(ssl.PROTOCOL_TLSv1) 
    company_addr_json = urlopen(req, context=ssltext).read().decode('utf8')
    addr = json.loads(company_addr_json)
    print(addr)
